...
function setFields($object, array $fieldArray = null)
{
    ...
    foreach ($fieldArray as $fldName => $fldValue) {
        ...
        if (!isset($typesmap[$fldName])) {
            ...
            trigger_error("SetFields error: parameter \"" . $fldName . "\" is not a valid parameter for an object of class \"" . $classname . "\", it will be omitted",
                          E_USER_WARNING);
            continue;
        }
        ...
        if ($fldValue === 'false') {
            ...
            if (substr($typesmap[$fldName], -2) == "[]") {
                trigger_error("Trying to assign an object into an array parameter \"" . $fldName . "\" of class \"" . $classname . "\", it will be omitted",
                              E_USER_WARNING);
                continue;
            }
            $obj = $typesmap[$fldName]::create();
            ...
            if (substr($typesmap[$fldName], -2) != "[]") {
                ...
                trigger_error("Trying to assign an array value into parameter \"" . $fldName . "\" of class \"" . $classname . "\", it will be omitted",
                              E_USER_WARNING);
                continue;
            }
